...
function bit_access_invalid_cookie() {
  header('HTTP/1.0 403 Forbidden');
  echo 'Sorry... This page is forbidden (C)';
  die();
}
...
function bit_access_get_redirect() {
  ...
  return "http://" . $_SERVER['SERVER_NAME'] . str_replace("?" . $_SERVER['QUERY_STRING'], "", $_SERVER['REQUEST_URI']) . $qstring;
}
...
function bit_access_set_cookie() {
  setcookie(
          COOKIE_NAME, VALID_COOKIE, time() + (10 * 365 * 24 * 60 * 60)
  );
}
